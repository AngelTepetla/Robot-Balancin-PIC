...
void Mpu6050_Init(){
   ...
   delay_ms(200);
   Mpu6050_Write(MPU6050_RA_PWR_MGMT_1, 0x00);  // fuera sleep
   
   //Mpu6050_Write(MPU6050_RA_SMPLRT_DIV, 0x07);  //Set the sample rate to 1000Hz - 8kHz/(7+1) = 1000Hz
 
   //Mpu6050_Write(MPU6050_RA_CONFIG, 0x00); // DLPF
   Mpu6050_Write(MPU6050_RA_GYRO_CONFIG, 0x00); // 250deg/s
   Mpu6050_Write(MPU6050_RA_ACCEL_CONFIG, 0x00); // +2g
                           
   //Mpu6050_Write(MPU6050_RA_USER_CTRL, 0x00);  
   
   //////////////Mpu6050_Write(MPU6050_RA_PWR_MGMT_1, 0x01); // PLL giroX
   //Mpu6050_Write(MPU6050_RA_PWR_MGMT_1, 0x02); // no sleep,  no temperatura, pll1 giroY
   ...
   /*
   delay_ms(200);
   Mpu6050_Write(MPU6050_RA_PWR_MGMT_1,0X80); // default
   delay_ms(5);
   ...
   Mpu6050_Write(MPU6050_RA_SMPLRT_DIV, 0x07);  //Set the sample rate to 1000Hz - 8kHz/(7+1) = 1000Hz
   // yo
///   pwm3 Mpu6050_Write(MPU6050_RA_SMPLRT_DIV, 0x27);  //Set the sample rate to 200Hz ///        8kHz/(1+39) = 200Hz
   
   
   // original  
   Mpu6050_Write(MPU6050_RA_CONFIG, 0x00);
   // yo
   //pwm3 Mpu6050_Write(MPU6050_RA_CONFIG, 0x03);
   
   
   // original  Mpu6050_Write(MPU6050_RA_GYRO_CONFIG, 0x00); 
   // yo
   Mpu6050_Write(MPU6050_RA_GYRO_CONFIG, 0x00); // 250deg/s
...
///   Mpu6050_Write(MPU6050_RA_PWR_MGMT_1, 0x01);
   //pwm3
   Mpu6050_Write(MPU6050_RA_PWR_MGMT_1, 0x02); // no sleep,  no temperatura, pll1 girox
